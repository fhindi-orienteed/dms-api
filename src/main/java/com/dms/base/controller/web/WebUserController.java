...
import com.dms.base.exception.PasswordNotMatchedException;
...
public class WebUserController extends UserController {
    ...
    public ResponseEntity<WebCreateUserResponse> createNewUser(@RequestBody CreateNewUserRequest newUserRequest) {
        ...
        newUserRequest.setPassword(hashedPassword);

        User newUser = userMapper.mapToCreateWebUserRequest(newUserRequest);
        User userResponse = userService.createNewUser(newUser);

        Driver newDriver = driverMapper.mpaToCreateWebDriver(newUserRequest);
        ...
        response.setDriver(responseDriver);
